...
class XMLSecurityDSig
{
  ...
  <ds:SignedInfo>
    ...
    {
        $doc =
            $objDoc instanceof DOMDocument ? $objDoc : $objDoc->ownerDocument;
        ...
        if ($xpath = $this->getXPathObj()) {
            ...
            if ($sinfo = $nodeset->item(0)) {
                ...
                if (!($canonNode = $nodeset->item(0))) {
                    $canonNode = $this->createNewSignNode(
    ...
    public function canonicalizeSignedInfo()
    {
        ...
        if ($doc) {
            ...
            if ($signInfoNode = $nodeset->item(0)) {
                ...
                $this->signedInfo = $this->canonicalizeData(
                    $signInfoNode,
                    $canonicalmethod,
                );
                        ...
                        if ($node->localName == "XPath") {
                            ...
                            foreach ($nslist as $nsnode) {
                                if ($nsnode->localName != "xml") {
                                    $arXPath["namespaces"][$nsnode->localName] =
                                        $nsnode->nodeValue;
                                }
                            }
                            break;
    ...
    private function addRefInternal(
        ...
        if (is_array($options)) {
            ...
            $prefix_ns = empty($options["prefix_ns"])
                ? null
                : $options["prefix_ns"];
            $id_name = empty($options["id_name"]) ? "Id" : $options["id_name"];
            $overwrite_id = !isset($options["overwrite"])
                ? true
                : (bool) $options["overwrite"];
            $force_uri = !isset($options["force_uri"])
                ? false
                : (bool) $options["force_uri"];
        }
        ...
        if (!$node instanceof DOMDocument) {
            ...
            if (!$overwrite_id) {
                $uri = $prefix_ns
                    ? $node->getAttributeNS($prefix_ns, $id_name)
                    : $node->getAttribute($id_name);
            }
            if (empty($uri)) {
    ...
    public function sign($objKey, $appendToNode = null)
    {
        ...
        if ($xpath = $this->getXPathObj()) {
            ...
            if ($sInfo = $nodeset->item(0)) {
                ...
                $sigValueNode = $this->createNewSignNode(
                    "SignatureValue",
                    $sigValue,
                );
                if ($infoSibling = $sInfo->nextSibling) {
                    $infoSibling->parentNode->insertBefore(
                        $sigValueNode,
                        $infoSibling,
                    );
                } else {
    ...
    public static function staticAdd509Cert(
        ...
        if (!$keyInfo) {
            ...
            $inserted = false;
            $keyInfo = $baseDoc->createElementNS(
                self::XMLDSIGNS,
                $dsig_pfx . "KeyInfo",
            );
        ...
        $x509DataNode = $baseDoc->createElementNS(
            self::XMLDSIGNS,
            $dsig_pfx . "X509Data",
        );
        ...
        foreach ($certs as $X509Cert) {
            if ($issuerSerial || $subjectName) {
                if (
                    ...
                    if (
                        $issuerSerial &&
                        !empty($certData["issuer"]) &&
                        !empty($certData["serialNumber"])
                    ) {
                        ...
                        $x509IssuerNode = $baseDoc->createElementNS(
                            self::XMLDSIGNS,
                            $dsig_pfx . "X509IssuerSerial",
                        );
            ...
            $x509CertNode = $baseDoc->createElementNS(
                self::XMLDSIGNS,
                $dsig_pfx . "X509Certificate",
                $X509Cert,
            );
