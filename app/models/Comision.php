...
class Comision
{
    ...
    public function obtenerMontoComision(
        $tipo_comision,
        $tipo_vehiculo,
        $usuario_id = null
    ) {
     ...
     */
    public function calcularComisionFinanciamiento(
        ...
        $planId = is_numeric($grupo_financiamiento)
            ? intval($grupo_financiamiento)
            : null;
        ...
        if ($rowPlan = $resultPlan->fetch_assoc()) {
            ...
            if (
                $rowPlan["aplica_comision"] == 1 &&
                $rowPlan["monto_comision"] !== null
            ) {
                return [
        ...
        error_log(
            "⚠️ COMISIÓN: Plan ID $planId no tiene comisión configurada en BD, usando valores por defecto",
        );
    ...
    public function obtenerComisiones(
        ...
        $sql = "SELECT
                    ...
                    CASE
                        ...
                        WHEN c.tipo_comision = 'venta' THEN
                            CONCAT('Comisión por venta - ', v.serie, ' - ', v.numero)
                ...
                LEFT JOIN ventas v ON (c.tipo_comision = 'venta' AND c.referencia_id = v.id_venta)
                WHERE 1=1
                AND (c.tipo_comision = 'inscripcion' OR c.tipo_comision = 'venta' OR (c.tipo_comision = 'financiamiento' AND COALESCE(f.estado_eliminado, 0) = 0 AND f.aprobado != 2))";
    ...
    public function obtenerDetalleComision(
        ...
        $sql = "SELECT
                    ...
                    v.fecha_emision as venta_fecha,
                    CASE
                        WHEN c.tipo_comision = 'inscripcion' THEN
                        ...
                        WHEN c.tipo_comision = 'venta' THEN
                            COALESCE(vcli.datos, 'Cliente no especificado')
                        ELSE 'N/A'
                    ...
                    CASE
                        ...
                        WHEN c.tipo_comision = 'venta' THEN
                            CONCAT('Nota de Venta: ', v.serie, ' - ', v.numero, ' - Total: ', v.moneda, ' ', v.total, ' - Fecha: ', DATE_FORMAT(v.fecha_emision, '%d/%m/%Y'))
                        ELSE c.observaciones
            ...
            LEFT JOIN ventas v ON (c.tipo_comision = 'venta' AND c.referencia_id = v.id_venta)
            LEFT JOIN clientes vcli ON v.id_cliente = vcli.id_cliente
            WHERE c.id_comision = ?
            AND (c.tipo_comision = 'inscripcion' OR c.tipo_comision = 'venta' OR (c.tipo_comision = 'financiamiento' AND COALESCE(f.estado_eliminado, 0) = 0 AND f.aprobado != 2))";
    ...
    {
        $sql =
            "UPDATE comisiones SET estado_comision = ? WHERE id_comision = ?";
        $stmt = $this->conectar->prepare($sql);
