...
function obtenerConductoresConCuotasVencidas() {
    ...
    // Procesar los resultados de las cuotas de productos
    ...
    usort($conductores_vencidos, function($a, $b) {
        return $b['num_cuotas'] - $a['num_cuotas'];
    });
    
    return $conductores_vencidos;
}

// Esta función genera el HTML para la lista de conductores
function generarListaConductoresVencidos($conductores_vencidos) {
    $html = '';
    $contador = 0;
    
    // Limitar a mostrar solo 3 conductores
    foreach ($conductores_vencidos as $index => $conductor) {
        if ($contador >= 3) break;
        
        // Determinar el color según el número de cuotas
        $color_class = $conductor['num_cuotas'] > 2 ? 'rgba(255, 86, 48, 0.1)' : 'rgba(255, 163, 48, 0.1)';
        $text_color = $conductor['num_cuotas'] > 2 ? '#FF5630' : '#FF9F1A';
        ...
        <div id="vencidos-item-'.($index+1).'" class="mb-3 p-3" style="background-color: rgba(247, 213, 74, 0.08); border-radius: 12px; transition: all 0.3s ease;">
            <div class="d-flex justify-content-between align-items-start">
                <div>
                    <h6 id="vencidos-nombre-'.($index+1).'" style="font-weight: 600; margin-bottom: 5px; color: #333;">'.$conductor['nombre'].'</h6>
                    <span id="vencidos-financiamiento-'.($index+1).'" style="font-size: 0.85rem; color: #8b8c64; display: block; margin-bottom: 8px;">Financiamiento: '.$conductor['tipo_financiamiento'].'</span>
                </div>
                <div class="text-end">
                    <span id="vencidos-cuotas-'.($index+1).'" class="px-2 py-1" style="background-color: '.$color_class.'; color: '.$text_color.'; font-weight: 600; border-radius: 6px; font-size: 0.85rem;">'.$conductor['num_cuotas'].' cuota'.($conductor['num_cuotas'] > 1 ? 's' : '').'</span>
                </div>
            </div>
            <div class="d-flex justify-content-between align-items-center mt-2">
                <span style="font-size: 0.85rem; color: #666;">Deuda total:</span>
                <h5 id="vencidos-monto-'.($index+1).'" style="font-weight: 700; color: #333; margin-bottom: 0;">'.$conductor['moneda'].' '.number_format($conductor['deuda_total'], 2, '.', ',').'</h5>  <!-- MODIFICADO: Usamos la moneda dinámica en lugar de S/ fijo -->
            </div>
        </div>';
...
$conductores_vencidos = obtenerConductoresConCuotasVencidas(); // Obtener la lista de conductores vencidos
$total_conductores = count($conductores_vencidos); // Contar los conductores obtenidos
$html_conductores = generarListaConductoresVencidos($conductores_vencidos); // Generar el HTML
?>
        ...
        <div class="card mini-stat bg-white text-dark" style="border-radius:20px;box-shadow:0 4px 6px -1px rgba(0,0,0,.1),0 2px 4px -1px rgba(0,0,0,.06); margin-top:-20px;">
                <div class="card-body">
                    <div class="mb-4">
                        ...
                        <h5 id="vencidos-titulo" class="fw-light text-uppercase text-black text-end" style="margin-bottom: 12px;">Conductores con Cuotas Vencidas</h5>
                        <h2 id="vencidos-contador" class="fw-bolder text-end" style="font-size: 1.8rem; margin-bottom: 20px;"><?= $total_conductores ?> conductores</h2>
                    </div>
                    ...
                    <div id="vencidos-lista" style="max-height: 250px; overflow-y: auto; margin-bottom: 12px;">
                        ...
                        <div class="text-center p-3">
                            <p style="color: #8b8c64;">No hay conductores con cuotas vencidas actualmente.</p>
                        </div>
                        <?php endif; ?>
